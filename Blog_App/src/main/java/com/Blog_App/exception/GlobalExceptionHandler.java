...
public class GlobalExceptionHandler {
	...
	@ResponseBody
	public ResponseEntity<ErrorFormat> handleBadRequestException(BadRequestException ex, WebRequest request) {
	...
	@ResponseStatus(HttpStatus.BAD_REQUEST)
	@ResponseBody
	public ResponseEntity<ErrorFormat> handleNotResourceFoundException(ResourceNotFoundException ex, WebRequest request) {
		logger.error("Not Found  exception occurred: {}", ex.getMessage(), ex);
		ErrorFormat ef = new ErrorFormat(LocalDateTime.now(), ex.getMessage(), request.getDescription(false));
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ef);
	}
}